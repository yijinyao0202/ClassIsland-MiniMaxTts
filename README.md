...
### 方式一：用 MiniMax API Console 操作

1. 打开 MiniMax API Console：https://solutions.minimaxi.com
2. 填写或选择你的 API Key。
3. 进入「音色管理」。
4. 按页面提示上传声音样本并创建复刻音色。
5. 如果页面要求先上传文件，可以先到「文件管理」上传音频。
6. 生成或复制最终的 `voice_id`。
7. 回到 ClassIsland，打开 `MiniMax Speech TTS` 配置。
8. 把 `voice_id` 填到「音色 ID」。
9. 点「保存自定义音色」，给它取一个容易认的名字。
10. 用「测试语音」确认能正常播报。

这个方式最适合普通用户：不用自己写代码，也能直接测试 MiniMax 的音色复刻和语音生成。

### 方式二：在 MiniMax 开放平台操作
